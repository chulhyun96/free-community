...
1. **회원 관리**(dummy 데이터 추가에 따른 후순위 작업)
    - [ ]  회원 가입 : 이메일 인증을 통한 계정 생성(이메일 중복 확인 및 유효성 검사)
    - [ ]  소셜 로그인 : 카카오톡 소셜 로그인 지원
    - [ ]  세션관리 : JWT로 세션 기반 인증으로 사용자의 로그인 상태 관리
    - [ ]  회원 정보 관리
        - [ ]  사용자 프로필 (닉네임, 이메일, 프로필 이미지) 조회 및 수정
        - [ ]  사용자 활동 이력 관리(최근 작성한 게시글/댓글 관리)
        - [ ]  차단 기능 : 비정상적인 활동에 대한 차단(다른 사용자들로 부터 신고를 N회 이상 당한 계정에 대한 조취)
2. 게시글 관리
    - [ ]  게시글 작성 / 수정 / 삭제
        - [ ]  글 작성 시 제목, 본문 텍스트, 이미지 파일 첨부 기능
        - [ ]  게시글 수정, 삭제 기능(삭제 시 ‘삭제 됨’으로 표시)
        - [ ]  자유 게시판의 성격을 띄는 만큼 익명성이 보장된다.
            - [ ]  게시글 작성 시 닉네임이 표기 된다.
    - [ ]  게시글 조회
        - [ ]  페이지네이션 및 무한 스크롤 방식으로 게시글 목록 조회(최신순, 인기순)
            - [ ]  페이징 쿼리 보조 인덱스 설정()
        - [ ]  조회수 어뷰징 방지 정책
            - [ ]  각 사용자는 게시글 1개당 10분에 1번 조회수가 집계된다.(10분동안 100번 조회하더라도 1번만 집계된다.) - 조회수 데이터 저장 성공 시 조회 내역이 없었음을 의미하기 때문에 조회수 1증가
